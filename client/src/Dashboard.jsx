...
function DashBoard({ user: initialUser, onLogout }) {
  ...
  const handleFileChange = async (e) => {
    ...
    for (const file of selectedFiles) {
      ...
      if (!allowedExtensions.includes(fileExtension)) {
        alert(`The .${fileExtension} file format is not supported. Skipping "${fileName}".`);
        continue;
      }
  ...
  const filteredFiles = useMemo(() => {
    return files.filter((file) => {
      ...
      if (selectedCategory === "images") return isImageFile(file.fileUrl || file.path, fileName);
      if (selectedCategory === "documents") return isDocFile(file.fileUrl || file.path, fileName);
      if (selectedCategory === "videos") return isVideoFile(file.fileUrl || file.path, fileName);
      if (selectedCategory === "audio") return isAudioFile(file.fileUrl || file.path, fileName);
      if (selectedCategory === "archives") return isArchiveFile(file.fileUrl || file.path, fileName);
  ...
  const handleGenerateShareLink = () => {
    if (!previewFile) return;
    const fileId = previewFile._id || previewFile.id;
    const expiryTimestamp = Date.now() + parseInt(shareExpiryDays, 10) * 86400000;
  ...
  return (
    <div className={`gemini-app-container ${isDarkMode ? "dark-theme" : "light-theme"}`}>
      <input
        type="file"
        multiple
        ref={fileInputRef}
      ...
      <main className="dashboard-wrapper">
        ...
        {isStorageWarningTriggered && (
          <div className="storage-warning-banner" role="alert">
            <div className="warning-banner-content">
              ...
              <span>
                <strong>Storage Capacity Warning:</strong> Your storage is over 90% full ({usedStorageMB} MB / {totalStorageMB} MB used). Please delete unnecessary files to free up space.
              </span>
            </div>
          </div>
        )}
          ...
          <div className="header-user-profile">
            <div className="user-avatar-pill">
              ...
              <div className="user-details-mini">
                ...
                {displayEmail && <span className="user-email-label">{displayEmail}</span>}
              </div>
            </div>
            ...
            <button className="gemini-logout-btn" type="button" onClick={onLogout}>
              Logout
            </button>
          </div>
        </header>
        ...
        <section className="gemini-hero">
          <div className="hero-text">
            ...
            <h1>
              Welcome back, <span className="gemini-gradient-text">{firstName}</span>
            </h1>
            <p className="hero-subtext">
              Manage your assets, track remaining quota, and organize files securely.
            </p>
        ...
        <section className="gemini-stats-grid">
          ...
          <div className="gemini-card">
            ...
            <p className="card-footnote">Encrypted cloud session operational.</p>
          </div>
        </section>
          ...
          {loadingFiles ? (
            ...
            <div className="vault-files-grid">
              {filteredFiles.map((file) => {
                const id = file._id || file.id;
                const fileName = file.originalName || file.filename || "Untitled File";
                ...
                return (
                  ...
                  >
                    ...
                    <div className="card-thumbnail-wrapper">
                      {thumbnail ? (
                        <img src={thumbnail} alt={fileName} className="card-thumbnail-img" />
        ...
        {isSelectionMode && selectedFileIds.length > 0 && (
          <div className="sticky-bottom-action-bar">
            <div className="sticky-bar-text">
              Selected <strong>{selectedFileIds.length}</strong> items ({selectedTotalMB} MB)
            </div>
            <button className="sticky-delete-all-btn" onClick={() => setShowBulkConfirmDelete(true)}>
              Delete All Selected
            </button>
          </div>
        )}
        ...
        {previewFile && (
          ...
          >
            <div className="modal-adaptive-shell" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header-bar">
                <h3 title={previewFile.originalName || previewFile.filename}>
                  {previewFile.originalName || previewFile.filename}
                </h3>
                <button className="modal-close-x" onClick={() => setPreviewFile(null)}>
                  ✕
                </button>
              </div>

              <div className="modal-media-fit-box">
                {isImageFile(previewFile.fileUrl || previewFile.path, previewFile.originalName) ? (
                  <img
                    src={previewFile.fileUrl || previewFile.path}
                    alt="Preview"
                    className="modal-fit-content"
                  />
                ) : isVideoFile(previewFile.fileUrl || previewFile.path, previewFile.originalName) ? (
                  <video controls autoPlay className="modal-fit-content">
                    <source src={previewFile.fileUrl || previewFile.path} />
                    Your browser does not support video playback.
                  </video>
                ) : isPdfFile(previewFile.fileUrl || previewFile.path, previewFile.originalName) ? (
                  <object
              ...
              <div className="modal-footer-bar">
                <span className="modal-size-text">
                  Size: {((previewFile.size || 0) / (1024 * 1024)).toFixed(2)} MB
                </span>
          ...
          <div className="modal-lightbox-backdrop backdrop-high-z">
            <div className="share-modal-box">
              <div className="modal-header-bar">
                ...
                <button className="modal-close-x" onClick={() => setShowShareModal(false)}>
                  ✕
                </button>
              </div>

              <p className="share-modal-description">
                Anyone with this link can view and download this file directly without logging in.
              </p>
              ...
              <div className="share-expiry-picker">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const fileId = previewFile._id || previewFile.id;
                    const expiryTimestamp = Date.now() + parseInt(e.target.value, 10) * 86400000;
                    const generatedToken = btoa(`${fileId}:${expiryTimestamp}`);
              ...
              <div className="share-url-box">
                <input type="text" readOnly value={generatedShareUrl} className="share-url-input" />
        ...
        {showConfirmDelete && (
          <div className="modal-lightbox-backdrop backdrop-high-z">
            <div className="confirm-alert-box">
              ...
              <p>
                This action cannot be undone. This file will be permanently removed from your Cloud Vault.
              </p>
        ...
        {showBulkConfirmDelete && (
          <div className="modal-lightbox-backdrop backdrop-high-z">
            <div className="confirm-alert-box">
              ...
              <p>
                Are you sure {selectedFileIds.length} files and {selectedTotalMB} MB amount of storage is going to be deleted?
              </p>
